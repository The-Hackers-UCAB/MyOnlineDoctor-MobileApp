...
class RequestAppointmentPage extends StatelessWidget {
  ...
  Widget _body(BuildContext context, AppointmentRequestState state) {
    if (state is AppointmentRequestStateInitial) {
      context
          .read<AppointmentRequestBloc>()
          .add(AppointmentRequestEventFetchBasicData());
    }

    return Stack(
      children: [
        if (state is! AppointmentRequestStateInitial)
          _appointmentRequestStreamBuilder(context),
        if (state is AppointmentRequestStateInitial ||
            state is AppointmentRequestStateLoading)
  ...
  Widget _appointmentRequestStreamBuilder(BuildContext builderContext) =>
      StreamBuilder<bool>(
          stream: builderContext
              .read<AppointmentRequestBloc>()
              .streamAppointmentRequest,
          ...
          children: [
            ...
            ButtonComponent(
                ...
                actionButton: () {
                  ...
                  AnaliticsService.logRequestAppointment(requestAppointment);
                  context.read<AppointmentRequestBloc>().add(
                      AppointmentRequestEventRequested(
  ...
  Widget _buildAppointmentTopInformation(BuildContext context) => Column(
        ...
        children: [
          ...
          doctor.gender == 'M'
              ? Text(
                  'Dr. ${doctor.firstName} ${doctor.firstSurname}',
                  style: const TextStyle(fontSize: 20),
                )
              : Text('Dra. ${doctor.firstName} ${doctor.firstSurname}',
                  style: const TextStyle(fontSize: 20))
        ],
      );
