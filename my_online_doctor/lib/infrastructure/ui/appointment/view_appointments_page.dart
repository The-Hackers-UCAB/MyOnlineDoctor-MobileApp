...
class ViewAppointmentsPage extends StatelessWidget {
  ...
  Widget _appointmentStreamBuilder(BuildContext builderContext) =>
      StreamBuilder<List<GetAppointmentModel>>(
          stream: builderContext.read<AppointmentBloc>().streamAppointment,
          builder: (BuildContext context,
              AsyncSnapshot<List<GetAppointmentModel>> snapshot) {
            if (snapshot.hasData) {
  ...
  Widget _renderMainBody(
          BuildContext context, List<GetAppointmentModel> data) =>
      Padding(
  ...
  Widget _renderAppointmentItem(
      BuildContext context, GetAppointmentModel item) {
    return Container(
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      item.doctor.gender == 'M'
                          ? Text(
                              'Dr. ${item.doctor.firstName} ${item.doctor.firstSurname}')
                          : Text(
                              'Dra. ${item.doctor.firstName} ${item.doctor.firstSurname}'),
                    ],
                  ),
